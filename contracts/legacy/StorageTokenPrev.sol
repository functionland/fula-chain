...
/// @title StorageTokenPrev
/// @notice STORAGE-LAYOUT REFERENCE ONLY — never deploy, never import from production code.
/// @dev Byte-for-byte copy of `contracts/core/StorageToken.sol` as of commit 0384d94, the last
///      commit before the LayerZero OFT bridge branch. The ONLY edit is the contract name, which
///      does not affect storage layout.
///
///      WHY THIS EXISTS: `.openzeppelin/` manifests are gitignored and no mainnet manifest exists,
///      so `upgrades.validateUpgrade` has no recorded baseline to compare the new implementation
///      against. `validateUpgrade` also accepts a reference *factory*, so this file supplies that
///      baseline offline — no RPC, no manifest. See `test/governance/integration/
///      StorageTokenLayout.test.ts` and the "Verification" section of `docs/bridge-design.md`
///      (check A).
///
///      SCOPE OF THE GUARANTEE: this proves the branch's diff is append-only and layout-compatible
///      with the code in THIS repo's history. It does NOT prove the live Ethereum/Base proxies run
///      that code — that is Phase 0 bytecode parity, which still requires network access
///      (`scripts/verifyStorageTokenImplParity.ts`) and remains open.
///
///      If a future upgrade changes StorageToken again, this file stays pinned at the LAST
///      DEPLOYED layout; do not re-baseline it against an undeployed intermediate.
contract StorageTokenPrev is
    GovernanceModule,
    ERC20Upgradeable,
