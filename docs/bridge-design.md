...
>
> **What changed during implementation** — the plan below is otherwise accurate, but four things
...
>    optimizer setting are recorded in the config.
>
...
>
> **Still open, and the first thing to do locally:** the Phase 0 verification below has NOT been
> run — every RPC host was blocked by egress policy in the environment this was built in. In
> particular, whether Ethereum and Base run the same implementation *code* is still unconfirmed
> (assumption 4 explains why the differing addresses prove nothing either way). Run
> `scripts/verifyStorageTokenImplParity.ts` on both networks and compare the printed keccak.
...
- **A (primary, no network needed):** `upgrades.validateUpgrade(LiveFactory, NewFactory, {kind:'uups', unsafeAllow:['constructor']})` using the `contracts/legacy/` reference source. `validateUpgrade` accepts a reference *factory*, so no manifest is required. If Phase 0 shows the Ethereum and Base bytecode is identical, one reference source suffices; if it differs, run this against **both**.
