...
describe("transferFromContract", function () {
  ...
  describe("bridgeBurn", function () {
    ...
    it("should correctly burn tokens through bridge", async function () {
      ...
      // The 5th argument is `block.timestamp` of the mined transaction. It was previously
      // asserted as `await time.latest() + 1`, which both reads the clock BEFORE the tx is mined
      // and hardcodes an assumption that exactly one second elapses — so it broke whenever an
      // earlier suite had moved time. Same defect as the `bridgeMint` test above. Match loosely,
      // then pin the exact value against the block the tx actually landed in.
      const tx = await storageToken
        .connect(bridgeOperator)
        .bridgeOp(BURN_AMOUNT, TARGET_CHAIN_ID, NONCE, 2);
      await expect(tx)
        .to.emit(storageToken, "BridgeOperationDetails")
        .withArgs(bridgeOperator.address, 2, BURN_AMOUNT, TARGET_CHAIN_ID, anyValue);

      const receipt = await tx.wait();
      const minedAt = (await ethers.provider.getBlock(receipt!.blockNumber))!.timestamp;
      await expect(tx)
        .to.emit(storageToken, "BridgeOperationDetails")
        .withArgs(bridgeOperator.address, 2, BURN_AMOUNT, TARGET_CHAIN_ID, minedAt);
  ...
  describe("Complete Token Lifecycle", function () {
    ...
    it("should execute complete token lifecycle process", async function () {
      ...
      // Timestamp matched loosely then pinned against the mined block — see the note on the
      // `bridgeMint` test above for why `await time.latest() + 1` was order-dependent.
      const mintTx = await storageToken
        .connect(bridgeOperator)
        .bridgeOp(MINT_AMOUNT, SOURCE_CHAIN_ID, NONCE, 1);
      await expect(mintTx)
        .to.emit(storageToken, "BridgeOperationDetails")
      ...
      const mintedAt = (await ethers.provider.getBlock(mintRcpt!.blockNumber))!.timestamp;
      await expect(mintTx)
        .to.emit(storageToken, "BridgeOperationDetails")
        .withArgs(bridgeOperator.address, 1, MINT_AMOUNT, SOURCE_CHAIN_ID, mintedAt);
